...
mutable struct ControlledHopOnStateAugmented
  ...
  uavstate::UAVState # RELATIVE to goal
...
@enum HOP_ACTION STAY=0 HOPOFF=-1

# NOTE
mutable struct HopOnAction
  action_idx::Int64
  uavaction::Union{Void,UAVAction}
...
function ControlledMultiRotorHopOnMDP(dynamics::UAVDynamicsModel)
...
function generate_sr(mdp::ControlledMultiRotorHopOnMDP, s::ControlledHopOnState, a::HopOnAction, rng::RNG=Base.GLOBAL_RNG) where {RNG <: AbstractRNG}
  ...
  if a.uavaction != nothing
    ...
    new_point::Point = Point(new_uavstate.x, new_uavstate.y)
    dyn_dist::Float64 = pointDist(old_point, new_point)

    if dyn_dist < EPSILON && sqrt(new_uavstate.xdot^2 + new_uavstate.ydot^2) < EPSILON
      # Hovering
